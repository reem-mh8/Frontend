...
@Component({
  selector: 'app-salle-de-sport-register',
  templateUrl: './salle-de-sport-register.component.html',
  styleUrls: ['./salle-de-sport-register.component.css']
})
export class SalleDeSportRegisterComponent {
  ...
  if (!data.nom || !data.adresse || !data.email || !data.mp || !data.mobile) {
    ...
    this.services.addSalleDeSport(salledesport).subscribe(
      res => {
        ...
        this.router.navigate(['/salle-de-sport-login']).then(() => { window.location.reload(); });
      },
      err => {
